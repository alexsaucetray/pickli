...
function levelPips(level: MacroDetail['level'], sentiment: MacroDetail['sentiment']) {
  const on = sentimentDot(sentiment);
  const off = 'bg-cream-200';
  return (
    <div className="flex gap-0.5">
      <div className={`h-1.5 w-3 rounded-full ${on}`} />
      <div className={`h-1.5 w-3 rounded-full ${level === 'moderate' || level === 'high' ? on : off}`} />
      <div className={`h-1.5 w-3 rounded-full ${level === 'high' ? on : off}`} />
    </div>
  );
}
...
export default function NutritionPanel({ macros, productCategory }: Props) {
  return (
    <div className="animate-fade-in space-y-3">
      <p className="text-[10px] font-medium text-pickle-500 text-center">
        For a <span className="font-bold text-pickle-700">{productCategory.toLowerCase()}</span>
      </p>

      <div className="bg-white border border-cream-200 rounded-2xl divide-y divide-cream-100 overflow-hidden">
        {macros.map((m, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-pickle-800 truncate">{m.name}</p>
                <div className="flex items-center gap-2 mt-1">
                  {levelPips(m.level, m.sentiment)}
                  <span className={`text-[9px] font-bold uppercase tracking-wider ${sl.cls}`}>
                    {sl.text}
                  </span>
                </div>
              </div>
        ...
        <div className="space-y-2 pt-1">
          {macros
            ...
            .map((m, i) => (
              <p
                key={i}
                className="text-[11px] text-pickle-500 leading-snug pl-3 border-l-2 border-brine-300"
              >
